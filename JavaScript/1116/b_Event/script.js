...
// ex) HTML 페이지에서 버튼 요소가 클릭되면 버튼 요소부터 시작해서 그의 부모요소들을 거쳐서 최상위 요소까지 전달

//? 이벤트 캡처링 ( 외부 -> 내부 요소 )
// : 이벤트 버블링과 반대방향으로 이벤트가 전달되는 현상
// , 최상위 요소에서 시작하여 이벤트가 발생한 요소까지 이벤트를 전달

// 이벤트의 전파 특징
// : 두 가지가 충동하는 경우 부모 요소들로 전파
// 캡처링 > 해당 대상 > 버블링
