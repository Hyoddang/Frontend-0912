...
- 윈도우(Windows)
  ...
  : 아래 경로에 해당하는 디렉터리 삭제
    윈도우 키 > 파일 탐색기
    C:\Program File\Nodejs
    C:\Program File (x86)\Nodejs
    C:\사용자\User\AppData\Roaming\npm

  : 윈도우 키 + r > cmd 입력 ( 명령 프롬프트 )
  node -v
  npm -v
...
? Windows에서 npm 실행이 되지 않을 경우
  (Node.js 설치 후에도 npm 실행이 안 되는 경우)

  윈도우 키 > 시스템 환경 변수 편집 > 시스템 속성 창 > 고급 > 환경 변수

  : 사용자 변수에서 Path 설정
    > '사용자 이름'에 대한 사용자 변수
    > C드라이브 > Users(사용자) > 자신의 컴퓨터 이름(User) > AppData > Roaming > npm

    Path(변수) > 환경 변수 편집 창 > 새로 만들기 > 복사한 npm 경로 삽입

  : 시스템 변수에서 Path 설정
  파일 탐색기 > C드라이브 > Program Files > nodejs 경로 복사

  > Path(변수) > 환경 변수 편집 > 새로만들기
  > 복사한 nodejs 경로 삽입
...
: 브라우저와 Node.js가 타입스크립트를 직접 실행할 수 없기 때문에 tsc로 코드 변환이 팔수
...
? 프로젝트 디렉토리 생성
  ...
  1) 원하는 위치에서 터미널 오픈
    mkdir 프로젝트명 (mkdir = make directory)
    mkdir TypeScript
    : 프로젝트 디렉토리 생성

    cd 프로젝트 이름 (cd = change directory)
    : 생성한 디렉토리로 이동
...
: ex) 출력 디렉토리 위치, 사용할 모듈 시스템, 지원하는 ECMAScript 버전 등을 관리
