document.addEventListener("DOMContentLoaded", () => {
  ...
  const finishButton = document.getElementById('completed-button')
    ...
    for (let i = 0; i < 12; i++) {
      ...
      <div class="card-inner">
        <div class="card-front">
          <img src="./images/NewspaperCat.jpg" alt="카드의 앞면" />
        </div>
  ...
  completedButton.addEventListener('click', () => {
    ...
    if (allFlipped) {
      ...
      alert(`게임 완료 ! 소요시간: ${Math.floor(gameTime / 1000)}`)
    } else {
      // 모든 카드가 뒤집히지 않았을 경우
      alert('완료되지 않았습니다.')
    }
  ...
  function toggleButtonVisibility(isGameStarted) {
    // 게임 시작 > 시작버튼 숨김 : 아니면 표시
    startButton.style.display = isGameStarted ? 'none' : 'block';
    // 게임 시작 > 리셋버튼 표시 : 아니면 숨김
    restartButton.style.display = isGameStarted ? 'block' : 'none';
    // 게임 시작 > 완료버튼 표시 : 아니면 숨김
